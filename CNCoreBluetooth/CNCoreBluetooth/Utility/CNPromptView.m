...
+(void)showStatusWithString:(NSString *)message withadjustBottomSpace:(float)space{
    ...
    showView.frame = CGRectMake((SCREENWIDTH - CGRectGetWidth(labelRect) - 20)/2, SCREENHEIGHT-(CGRectGetHeight(labelRect)+10)-49-iPhoneXBottomPara-edgeDistancePage-space, CGRectGetWidth(labelRect)+20, CGRectGetHeight(labelRect)+10);
    [UIView animateWithDuration:2 animations:^{
        showView.alpha = 0;
    } completion:^(BOOL finished) {
        [showView removeFromSuperview];
