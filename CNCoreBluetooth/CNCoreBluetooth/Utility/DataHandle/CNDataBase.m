...
- (void)initDataBase{
    ...
    [_db open];
    // 初始化数据表
    NSString *periSql = @"Create table if not exists peripheral (id INTEGER primary key autoincrement  not null , peri_id VARCHAR(255), peri_name VARCHAR(255), peri_isPwd INTEGER, peri_isTouchUnlock INTEGER)";
...
-(void)addPeripheralInfo:(CNPeripheralModel *)model{
    [self.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        if (totalCount==0) {
            //插入
            [db executeUpdate:@"INSERT INTO peripheral (peri_id, peri_name, peri_isPwd, peri_isTouchUnlock) VALUES (?, ?, ?, ?)",model.periID, model.periname, @(model.isPwd), @(model.isTouchUnlock)];
        }
    }];
}
...
-(void)updatePeripheralInfo:(CNPeripheralModel *)model{
    [self.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        if (totalCount > 0) {
            //更新
            [db executeUpdate:@"UPDATE peripheral SET peri_id = ?, peri_name = ?, peri_isPwd = ?, peri_isTouchUnlock = ? WHERE peri_id = ?",model.periID, model.periname, @(model.isPwd), @(model.isTouchUnlock), model.periID];
